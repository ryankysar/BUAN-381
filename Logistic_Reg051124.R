...
set.seed(678)
Sets <- partition(Prices$Price, p = c(train = 0.70, Hold = 0.15, test = 0.15))
str(Sets)
...
#builds the confusion matrix to look at accuracy on testing data in-of-sample
...
# Accuracy from Probit Regression on training data in-of-sample
...
########################## Create an ROC Curve ##########################
##############################################################################
Train$Basement <- as.factor(ifelse(Train$Basement == 1, 1, 0))

Model_LOG <- glm(Basement ~ Total_Area + Bedrooms + Stories + Mainroad + GuestRoom + HotWaterHeating + AC + Parking_Spots + PrefArea,
                 data = Train, family = binomial(link = "logit"), maxit = 1000)

pred_prob_logit <- predict(Model_LOG, Train, type = "response")

roc_curve_data_logit <- roc(Train$Basement, pred_prob_logit)

plot(roc_curve_data_logit, col = "blue", main = "ROC Curve - Logistic Regression", 
     xlab = "False Positive Rate", ylab = "True Positive Rate")

abline(a = 0, b = 1, lty = 2, col = "red")

auc_logit <- auc(roc_curve_data_logit)
...
TABLE_VAL_3 <- as.table(matrix(c(RAND_IN, RAND_OUT, SVM_IN, SVM_OUT, SVM_IN_TUNE, SVM_OUT_TUNE, LOG_IN, LOG_OUT, PROB_IN, PROB_OUT), ncol=1, byrow=TRUE))

# Naming columns and rows
colnames(TABLE_VAL_3) <- c('Value')
rownames(TABLE_VAL_3) <- c('RAND_IN', 'RAND_OUT', 'SVM_IN', 'SVM_OUT', 'SVM_IN_TUNE', 'SVM_OUT_TUNE', 'LOG_IN', 'LOG_OUT', 'PROB_IN', 'PROB_OUT')
...
# Create a bar plot
barplot(accuracies, names.arg = model_names, las = 2, col = "skyblue", 
        main = "Model Accuracies", xlab = "Models", ylab = "Accuracy")
...
class_fmla <- formula(Parking_Spots ~ .)  

class_tree <- class_spec %>%
  fit(formula = class_fmla, data = Train)

rpart.plot(class_tree$fit, type = 4, extra = 2, roundint = FALSE)
...
############### Make a table with accurcies from model ###########################

# Create a matrix with accuracy values
accuracy_matrix <- matrix(c(M_RAND_OUT, M_Rand_IN, CART_M_IN, CART_M_OUT, 
                            SVM_IN_M_TUNE, SVM_OUT_M_TUNE, SVM_M_IN, SVM_M_OUT), 
                          ncol = 1, byrow = TRUE)

# Convert the matrix to a table
accuracy_table <- as.table(accuracy_matrix)

# Naming columns and rows
colnames(accuracy_table) <- c('Value')
rownames(accuracy_table) <- c('M_RAND_OUT', 'M_Rand_IN', 'CART_M_IN', 'CART_M_OUT', 
                              'SVM_IN_M_TUNE', 'SVM_OUT_M_TUNE', 'SVM_M_IN', 'SVM_M_OUT')

# Print the table
print(accuracy_table)

############ Make a plot with accuracies from model #############
...
accuracy_values <- c(M_RAND_OUT, M_Rand_IN, CART_M_IN, CART_M_OUT, 
                     SVM_IN_M_TUNE, SVM_OUT_M_TUNE, SVM_M_IN, SVM_M_OUT)

# Names of the models
model_names <- c('M_RAND_OUT', 'M_Rand_IN', 'CART_M_IN', 'CART_M_OUT', 
                 'SVM_IN_M_TUNE', 'SVM_OUT_M_TUNE', 'SVM_M_IN', 'SVM_M_OUT')

# Create a bar plot
barplot(accuracy_values, 
        names.arg = model_names,
        col = rainbow(length(accuracy_values)),  # Color bars differently
        main = "Accuracy of Different MultiClass Models ",
        xlab = "Model type",
        ylab = "Accuracy in %'s",
        ylim = c(0, 1),  # Set y-axis limits
        cex.names = 0.42)  # Adjust the size of x-axis labels
