...
###################### SPLINE #######################
M6 <- gam(Price ~ s(Total_Area), data = Train, family = 'gaussian')
summary(M6)
...
########################LASSO regularization to SPLINE##########################
M6_lasso <- gam(Price ~ s(Total_Area), data = Train, family = gaussian, method = "REML", select = TRUE)

# Print model summary
summary(M6_lasso)

# Make predictions on the training and testing datasets
PRED_6_IN_lasso <- predict(M6_lasso, Train, type = 'response')
PRED_6_OUT_lasso <- predict(M6_lasso, Test, type = 'response')

# Calculate RMSE for the training dataset
RMSE_6_IN_lasso <- sqrt(mean((PRED_6_IN_lasso - Train$Price)^2))

# Calculate RMSE for the testing dataset
RMSE_6_OUT_lasso <- sqrt(mean((PRED_6_OUT_lasso - Test$Price)^2))
...
predictions_plot <- ggplot(predictions_IN, aes(x = Actual)) +
  geom_point(aes(y = PRED_1_IN, color = "PRED_1_IN"), shape = 1, alpha = 0.2) +
  geom_point(aes(y = PRED_2_IN, color = "PRED_2_IN"), shape = 2, alpha = 0.3) +
  geom_point(aes(y = PRED_3_IN, color = "PRED_3_IN"), shape = 3, alpha = 0.8) +
  geom_point(aes(y = PRED_4_IN, color = "PRED_4_IN"), shape = 4, alpha = 0.8) +
  geom_point(aes(y = PRED_5_IN, color = "PRED_5_IN"), shape = 5, alpha = 0.5) +
  geom_point(aes(y = pred_train, color = "pred_train"), shape = 6, alpha = 0.5) +
  geom_point(aes(y = PRED_6_IN, color = "PRED_6_IN"), shape = 7, alpha = 0.5) +
  geom_vline(xintercept = 0) +  # Add vertical line at x = 0
  scale_color_manual(values = c("PRED_1_IN" = "black", "PRED_2_IN" = "blue", "PRED_3_IN" = "red", "PRED_4_IN" = "purple", "PRED_5_IN" = "orange", "pred_train" = "pink", "PRED_6_IN" = "brown"), 
                     guide = "legend") +  # Set colors and legend
  labs(x = "Actual", y = "Predicted") +  # Set axis labels
  ggtitle("Predicted vs. Actual Prices") +  # Add a title
  theme_minimal() +  # Apply a minimal theme
  scale_x_continuous(labels = scales::number_format()) +  # Prevent scientific notation on x-axis
  scale_y_continuous(labels = scales::number_format()) +  # Prevent scientific notation on y-axis
  theme(legend.position = "bottom", legend.justification = "right")  # Position the legend
...
reg_mult_mod <- lmridge(Price ~ Total_Area + Bedrooms + Bathrooms + Stories + Parking_Spots, Train, lambda = seq(0, 1, 0.5))

# Summary of the Ridge regression model
summary(reg_mult_mod)

# Predictions on training data
PRED_reg_mult_mod_IN <- predict(reg_mult_mod, newdata = Train)
...
PRED_reg_mult_mod_OUT <- predict(reg_mult_mod, newdata = Test)
RMSE_reg_mult_OUT <- sqrt(mean((PRED_reg_mult_mod_OUT - Test$Price)^2))

RMSE_reg_mult_OUT


################################## Tuned Regularized Model ############### 3 #############
...
PRED_reg_mult_mod_OUT_ <- predict(reg_mult_mod, s = best_lambda, newx = model.matrix(fmla, data = Test))



RMSE_reg_mult_IN_T <- sqrt(mean((PRED_reg_mult_mod_IN - Train$Price)^2))
RMSE_reg_mult_OUT_T <- sqrt(mean((PRED_reg_mult_mod_OUT - Test$Price)^2))

RMSE_reg_ridge_mod_IN

RMSE_reg_ridge_mod_OUT
...
RMSE_SVM_IN_TUNE <- sqrt(mean((PRED_Mult_SVM_IN - Train$Price)^2))
RMSE_SVM_OUT_TUNE <- sqrt(mean((PRED_Mult_SVM_OUT - Test$Price)^2))
...
# Create the matrix with RMSE values
TABLE_VAL_MULT <- matrix(c(RMSE_Mult_M1_IN,RMSE_Mult_M1_OUT, 
                           RMSE_reg_mult_IN, RMSE_reg_mult_OUT, 
                           RMSE_reg_ridge_mod_IN, RMSE_reg_ridge_mod_OUT, 
                           RMSE_SPl_MULT_IN, RMSE_Spl_MULT_OUT, 
                           RMSE_SVM_IN, RMSE_SVM_MULT_OUT, 
                           RMSE_SVM_IN_TUNE, RMSE_SVM_OUT_TUNE, 
                           rmse_in_bag, rmse_out_bag, 
                           rmse_bag_tuned, rmse_bag_tuned_o),
                         ncol = 2, byrow = TRUE)

# Define column and row names
colnames(TABLE_VAL_MULT) <- c('IN', 'OUT')
rownames(TABLE_VAL_MULT) <- c('Model 1', 'Regularized', 'Tuned Regularization', 'Spline', 
                              'SVM', 'SVM TUNE', 'Bagged Tree', 'Tuned Bagged')
...
#FIT THE FINALIZED MODEL
final_model_Hold <- final_specc %>% fit(fmla, Hold)

pred_class_bag_tune_hold <- predict(final_model_Hold, new_data = Hold) %>%
  bind_cols(Hold$Price)

#RMSE Tuned
rmse_bag_tuned_Hold <- sqrt(mean((pred_class_bag_tune_hold$...2 - pred_class_bag_tune_hold$.pred)^2))
