...
#Benchmark the LM model against the Bivariate (M4) Model, using R^2
r_squared_linear <- summary(M1)$r.squared
r_squared_polynomial <- summary(M4)$r.squared

cat("Linear Model R-squared:", r_squared_linear, "\n")
cat("Polynomial Model R-squared:", r_squared_polynomial, "\n")
...
#Regularize the bivariate model (M2,3,4)?

#Tune the regularization parameter to improve E_OUT?
...
M7 <- glm(Price ~ Total_Area, data = Train, family = poisson())
summary(M7)
PRED_7_IN <- predict(M7, Train, type = 'response')
PRED_7_OUT <- predict(M7, Test, type = 'response')

(RMSE_7_IN<-sqrt(sum((PRED_7_IN-Train$Price)^2)/length(PRED_7_IN)))  
(RMSE_7_OUT<-sqrt(sum((PRED_7_OUT-Test$Price)^2)/length(PRED_7_OUT))) 

#Model Comparisons
TABLE_VAL_2 <- as.table(matrix(c(RMSE_1_IN, RMSE_2_IN, RMSE_3_IN, RMSE_4_IN, RMSE_5_IN, RMSE_6_IN, RMSE_7_IN, RMSE_1_OUT, RMSE_2_OUT, RMSE_3_OUT, RMSE_4_OUT, RMSE_5_IN, RMSE_6_OUT, RMSE_7_OUT), ncol=7, byrow=TRUE))
colnames(TABLE_VAL_2) <- c('LINEAR', 'QUADRATIC', 'CUBIC', '4th ORDER','QUASIPOISSON', 'SPLINE', 'POISSON')
rownames(TABLE_VAL_2) <- c('RMSE_IN', 'RMSE_OUT')
TABLE_VAL_2 #REPORT OUT-OF-SAMPLE ERRORS FOR BOTH HYPOTHESIS


# Create a dataframe combining training and holdout partitions
combined_data <- rbind(Train, Hold)

# Create a ggplot object
p <- ggplot() +
  # Add training partition points
  geom_point(data = Train, aes(x = Total_Area, y = Price, color = "Training Data"), size = 3) +
  # Add holdout partition points
  geom_point(data = Hold, aes(x = Total_Area, y = Price, color = "Holdout Data"), size = 3) +
  # Add linear model
  geom_smooth(data = combined_data, aes(x = Total_Area, y = predict(M1, newdata = combined_data), color = "Linear Model"), method = "lm", formula = y ~ x, se = FALSE, linetype = "dashed") +
  # Add quadratic model
  geom_smooth(data = combined_data, aes(x = Total_Area, y = predict(M2, newdata = combined_data), color = "Quadratic Model"), method = "lm", formula = y ~ poly(x, 2), se = FALSE) +
  # Add cubic model
  geom_smooth(data = combined_data, aes(x = Total_Area, y = predict(M3, newdata = combined_data), color = "Cubic Model"), method = "lm", formula = y ~ poly(x, 3), se = FALSE) +
  # Add 4th polynomial model
  geom_smooth(data = combined_data, aes(x = Total_Area, y = predict(M4, newdata = combined_data), color = "4th Polynomial Model"), method = "lm", formula = y ~ poly(x, 4), se = FALSE) +
  # Add quasipoisson model
  stat_smooth(data = combined_data, aes(x = Total_Area, y = Price, color = "Quasipoisson Model"), method = "glm", formula = y ~ x, se = FALSE, method.args = list(family = quasipoisson())) +
  # Add spline model
  geom_smooth(data = combined_data, aes(x = Total_Area, y = predict(M6, newdata = combined_data), color = "Spline Model"), method = "gam", formula = y ~ s(x, bs = "cs"), se = FALSE) +
  # Add poisson model
  stat_smooth(data = combined_data, aes(x = Total_Area, y = Price, color = "Poisson Model"), method = "glm", formula = y ~ x, se = FALSE, method.args = list(family = poisson(link = "log"))) +
  # Add legend
  labs(color = "Model") +
  scale_color_manual(values = c("blue", "red", "green", "purple", "orange", "yellow", "pink", "black", "grey"),
                     labels = c("Training Data", "Holdout Data", "Linear Model", "Quadratic Model", "Cubic Model", "4th Polynomial Model", "Quasipoisson Model", "Spline Model", "Poisson Model")) +
  theme_minimal()

# Print the plot
print(p)

#Table Summarizing E_IN & E_OUT for each of models
TABLE_VAL_2 <- as.table(matrix(c(RMSE_1_IN, RMSE_2_IN, RMSE_3_IN, RMSE_4_IN, RMSE_5_IN, RMSE_6_IN, RMSE_7_IN, RMSE_1_OUT, RMSE_2_OUT, RMSE_3_OUT, RMSE_4_OUT, RMSE_5_IN, RMSE_6_OUT, RMSE_7_OUT), ncol=7, byrow=TRUE))
colnames(TABLE_VAL_2) <- c('LINEAR', 'QUADRATIC', 'CUBIC', '4th ORDER','QUASIPOISSON', 'SPLINE', 'POISSON')
rownames(TABLE_VAL_2) <- c('RMSE_IN', 'RMSE_OUT')
TABLE_VAL_2 #REPORT OUT-OF-SAMPLE ERRORS FOR BOTH HYPOTHESIS
