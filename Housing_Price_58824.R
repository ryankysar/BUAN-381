...
#Bivariate Plot of all models against training and holdout partitions, all on same diagram
ggplot() +
  #training partition points
  ...
  # Add linear model
  geom_smooth(data = Prices, aes(x = Total_Area, y = predict(M1, newdata = Prices), color = "Linear Model"), method = "lm", se = FALSE, linetype = "dashed") +
  # Add quadratic model
  geom_smooth(data = Prices, aes(x = Total_Area, y = predict(M2, newdata = Prices), color = "Quadratic Model"), method = "lm", se = FALSE) +
  # Add cubic model
  geom_smooth(data = Prices, aes(x = Total_Area, y = predict(M3, newdata = Prices), color = "Cubic Model"), method = "lm", se = FALSE) +
  # Add 4th polynomial model
  geom_smooth(data = Prices, aes(x = Total_Area, y = predict(M4, newdata = Prices), color = "4th Polynomial Model"), method = "lm", se = FALSE) +
  # Add quasipoisson model
  geom_smooth(data = Prices, aes(x = Total_Area, y = predict(M5, newdata = Prices), color = "Quasipoisson Model"), method = "lm", se = FALSE) +
  # Add spline model
  geom_smooth(data = Prices, aes(x = Total_Area, y = predict(M6, newdata = Prices), color = "Spline Model"), method = "lm", se = FALSE) +
  # Add poisson model
  geom_smooth(data = Prices, aes(x = Total_Area, y = predict(M7, newdata = Prices), color = "Poisson Model"), method = "lm", formula = y ~ x + I(x^2), se = FALSE) +
  # Add legend
  labs(color = "Model") +
  scale_color_manual(values = c("black", "grey", "blue", "red", "green", "purple", "orange", "yellow", "pink"),
