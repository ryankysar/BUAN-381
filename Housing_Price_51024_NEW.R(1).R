...
RMSE_reg_IN <- sqrt(mean((pred_train - y_train)^2))
RMSE_reg_OUT <- sqrt(mean((pred_test - y_test)^2))

cat("In-sample RMSE:", RMSE_reg_IN, "\n")
cat("Out-of-sample RMSE:", RMSE_reg_OUT, "\n")

results <- data.frame(
  Metric = c("In-sample RMSE", "Out-of-sample RMSE"),
  Value = c(RMSE_reg_IN, RMSE_reg_OUT)
)
print(results)
...
#Model Comparisons
TABLE_VAL_2 <- as.table(matrix(c(RMSE_1_IN, RMSE_2_IN, RMSE_3_IN, RMSE_4_IN,RMSE_reg_IN, RMSE_5_IN, RMSE_6_IN, RMSE_1_OUT, RMSE_2_OUT, RMSE_3_OUT, RMSE_4_OUT, RMSE_reg_OUT, RMSE_5_OUT, RMSE_6_OUT), ncol=7, byrow=TRUE))
colnames(TABLE_VAL_2) <- c('LINEAR', 'QUADRATIC', 'CUBIC', '4th ORDER', 'Regularized','QUASIPOISSON', 'SPLINE')
...
plot(Train$Price ~ Train$Total_Area, col='black')
predictions_1 <- predict(M1, list(Total_Area=x_grid))
predictions_2 <- predict(M2, list(Total_Area=x_grid, Total_Area2=x_grid^2))
predictions_3 <- predict(M3, list(Total_Area=x_grid, Total_Area2=x_grid^2, Total_Area3=x_grid^3))
predictions_4 <- predict(M4, list(Total_Area=x_grid, Total_Area2=x_grid^2, Total_Area3=x_grid^3, Total_Area4=x_grid^4))
predictions_5 <- predict(M5, list(Total_Area=x_grid), type = 'response')
predictions_6 <- predict(M6, list(Total_Area=x_grid), type = 'response')
lines(x_grid, predictions_1, col='blue', lwd=3) #PLOTS M1 LINEAR MODEL
lines(x_grid, predictions_2, col='lightgreen', lwd=3) #PLOTS M2 QUADRATIC MODEL
lines(x_grid, predictions_3, col='green', lwd=3) #PLOTS M3 CUBIC MODEL
lines(x_grid, predictions_4, col='darkgreen', lwd=3) #PLOTS M4 4TH ORDER MODEL
lines(x_grid, predictions_5, col='yellow', lwd=3) #PLOTS M5 QUASI POISSON MODEL
lines(x_grid, predictions_6, col='orange', lwd=3) #PLOTS M6 SPLINE MODEL
points(Test$Price ~ Test$Total_Area, col='red', pch=3, cex=.5)

#THE LINES ARE NOT SHOWING UP IN THE PLOT, DON"T KNOW WHAT"S WRONG. HAVE BEEN TROUBLE SHOOTING IT BUT NO SUCCESS
...
spec_rf <- rand_forest(min_n = 20 , #minimum number of observations for split
  ...
  set_mode("regression") %>% #can set to classification for binary prediction
...
set.seed(456) #NEED TO SET SEED WHEN FITTING OR BOOTSTRAPPED SAMPLES WILL CHANGE
...
pred_in_Rand <- predict(random_forest, new_data = Train) %>%
  bind_cols(Train$Price) #ADD CLASS PREDICTIONS DIRECTLY TO TEST DATA

# Calculate RMSE
rmse_in_Rand <- sqrt(mean((pred_in_Rand$...2 - pred_in_Rand$.pred)^2))
rmse_in_Rand



#GENERATE OUT-OF-SAMPLE PREDICTIONS ON THE TEST SET AND COMBINE WITH TEST DATA
pred_out_Rand <- predict(random_forest, new_data = Test) %>%
  bind_cols(Test$Price) #ADD CLASS PREDICTIONS DIRECTLY TO TEST DATA

# Calculate RMSE
rmse_out_Rand <- sqrt(mean((pred_out_Rand$...2 - pred_out_Rand$.pred)^2))
rmse_out_Rand
